...
dirt_csv_dirt = dirt_base + "round_1/round_2_inconsistent/round_3/round_4/"#dirt of compare file
out_put_dirt = dirt_base + "round_1/round_2_inconsistent/round_3/round_4/round_5/"#dirt of output dirt, including cated fastq, assembly, and fasta files
temp_work_dirt = dirt_base + "round_1/round_2_inconsistent/round_3/round_4/round_5/temp/"# temp dirt used to cat fastq files
file_name = '4th_round_cmp.csv'
...
with open(dirt_base + "round_1/round_2_inconsistent/cluster_members_1st.json") as input:
        cluster_result_readin_1st = json.load(input)

with open(dirt_base + "round_1/round_2_inconsistent/round_3/cluster_members_2nd.json") as input:
        cluster_result_readin_2nd = json.load(input)

with open(dirt_base + "round_1/round_2_inconsistent/round_3/round_4/cluster_members_3rd.json") as input:
        cluster_result_readin_3rd = json.load(input)
...
for j in range(max(assignments)):
        ...
        for i in a_temp_list:
                for k in cluster_result_readin_3rd[i[:-len('.fasta')]]:
                        for h in cluster_result_readin_2nd[k]:
				for g in cluster_result_readin_1st[h]:
                                	a_temp_list_of_barcode += [g]
        #checked_barcodes += len(a_temp_list_of_barcode)
